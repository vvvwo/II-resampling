/*********************************************************************************
                      ...
                      CGAL Remeshing with edge reconstruction

                             Updating in 2020/09/10

                                By Dr. Chenlei Lv

            The functions includes:
            CGAL Advancing-based remesh;
...
class CGAL_Advancing_R_Edge {//remain edge

public: 

    std::vector<std::vector<double>> pointsNewR;
    std::vector<std::vector<int>> faceInforsR;
...
public:
    ...
    void CGAL_Advancing_R_Edge_init(std::vector<std::vector<double>> point_input) {
        pointsR = point_input;        
    }

    void CGAL_Advancing_R_Edge_FaceInfor() {

        // Points with normals.
        Pwn_vector points;
        for (int i = 0; i < pointsR.size(); i++) {
            Kernel::Point_3 pi(pointsR[i][0], pointsR[i][1], pointsR[i][2]);
            Kernel::Vector_3 ni(pointsR[i][3], pointsR[i][4], pointsR[i][5]);
            Point_with_normal pni(pi, ni);
            points.push_back(pni);
        }
        pointsR.clear();       
        ...
        if (pointsNewR.size() > 0) {
            pointsNewR.clear();        
        }

        if (faceInforsR.size() > 0) {
            faceInforsR.clear();        
        }

        for (std::size_t i = 0; i < structured_pts.size(); ++i) {

            std::vector<double> pii(3);
            pii[0] = structured_pts[i].first[0];
            pii[1] = structured_pts[i].first[1];
            pii[2] = structured_pts[i].first[2];
            pointsNewR.push_back(pii);
        
        }
           
        for (std::size_t i = 0; i < output.size(); ++i) {
            std::vector<int> faceInforsR_i(3);
            faceInforsR_i[0] = output[i][0];
            faceInforsR_i[1] = output[i][1];
            faceInforsR_i[2] = output[i][2];
            faceInforsR.push_back(faceInforsR_i);
        }           
    }
