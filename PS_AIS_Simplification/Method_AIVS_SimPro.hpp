...
public:
	...
	void AIVS_Pro_init_Edge(BallRegion brInput, string fileNameInput) {//fileNameInput is the global path which has removed the tail:Remesh\Bunny
	...
	vector<vector<double>> AIVS_simplification_Curvature(int pointNum, vector<double> prate) {
		...
		//5. the simplification number for each box			
		AIVS_Classification_Curvature(prate);

		vector<vector<int>> boxSimiNumer = AIVS_BoxSimplification_Points_UserDefined(pointNum);
		//6. the points infor in each box
		br.squareBoxes;
		...
		//AIVS_Voroni_OpenMP; AIVS_Voroni_Cuda(;AIVS_Voroni_OpenMP_KNN.
		AIVS_Pipeline_UserDefine_OpenMP(
			boxIndexNumberCu,
			boxcenter,
			boxcenterIndex,
			boxScale,
		...
		//Final Step, to achieve the accurate points number.
		vector<vector<double>> finalResult = AIVS_AccurateCut_Optimization(pointNum, true);//final simplification result.
		return finalResult;
	}
		
	vector<vector<double>> AIVS_simplification_Edge_Curvature(int pointNum, double c1N, double c2E) {
		...
		//Final Step, to achieve the accurate points number.
		vector<vector<double>> finalResult = AIVS_AccurateCut_Optimization(SimResult, pointNum);//final simplification result.
		vector<vector<double>> finalResult2 = AIVS_AddEdgePoints(finalResult);
...
private:
	...
	//For curvature estimation with mesh
	void AIVS_NormalValue_Meshbased() {//average angle estimation

		CGAL_Advancing_Reconstruct cr;
		cr.CGAL_Advancing_Remesh_init(br.pointCloudData);
		vector<vector<int>> faceInforN = cr.CGAL_Advancing_Remesh_FaceInfor();		

		vector<vector<double>> point_M = br.pointCloudData;

		vector<vector<int>> neighbor_M(point_M.size());

		for (int i = 0; i < faceInforN.size(); i++) {
			int b1 = faceInforN[i][0];
			int b2 = faceInforN[i][1];
			int b3 = faceInforN[i][2];
			neighbor_M[b1].push_back(b2);
			neighbor_M[b1].push_back(b3);
			neighbor_M[b2].push_back(b3);
			neighbor_M[b2].push_back(b1);
			neighbor_M[b3].push_back(b1);
			neighbor_M[b3].push_back(b2);
		}
...
#pragma omp parallel for
		for (int i = 0; i < point_M.size(); i++) {
			if (i % 10000 == 0) {
				cout << (point_M.size() - i) / 10000 << ",";
			}
			vector<double> pTemp = point_M[i];
			vector<int> face_i = neighbor_M[i];
			vector<vector<double>> normal_i;
			for (int j = 0; j < face_i.size() / 2; j++) {
				int b2 = face_i[2 * j];
				int b3 = face_i[2 * j + 1];
				vector<double> pTemp2 = point_M[b2];
				vector<double> pTemp3 = point_M[b3];
	...
	//For curvature estimation with mesh
	void AIVS_NormalValue_Meshbased_HaveObj() {//average angle estimation

		string fileNameObj = fileName + ".obj";
		char* p = new char[strlen(fileNameObj.c_str()) + 1];
		strcpy(p, fileNameObj.c_str());
		GLMmodel* pModelM = glmReadOBJ(p);
		glmFacetNormals(pModelM);

		if (pModelM->numvertices != br.pointCloudData.size()) {
			cout << "Error, point number not equal!";		
		}

		vector<vector<double>> point_M;		

		for (int i = 1; i <= pModelM->numvertices; i++) {
			vector<double> pi(3);
			pi[0] = pModelM->vertices[3 * i];
			pi[1] = pModelM->vertices[3 * i + 1];
			pi[2] = pModelM->vertices[3 * i + 2];
			point_M.push_back(pi);			
		}	

		vector<vector<int>> neighbor_M(point_M.size());

		for (int i = 0; i < pModelM->numtriangles; i++) {
			int b1 = pModelM->triangles[i].vindices[0] - 1;
			int b2 = pModelM->triangles[i].vindices[1] - 1;
			int b3 = pModelM->triangles[i].vindices[2] - 1;
			neighbor_M[b1].push_back(b2);
			neighbor_M[b1].push_back(b3);
			neighbor_M[b2].push_back(b3);
			neighbor_M[b2].push_back(b1);
